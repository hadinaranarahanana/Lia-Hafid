...
(function () {
  ...
  function getGalleryImages() {
    return CONFIG.galleryFiles.map((fileName) => `${CONFIG.galleryPath}${fileName}`);
  }
  ...
  async function prepareMainContent() {
  ...
  function initLenis() {
    ...
    // Use native scrolling to preserve mouse wheel behavior on desktop.
    // The page layout already supports independent right-panel scroll.
  ...
  function initGallerySwiper() {
    ...
    images.forEach((src, i) => {
      ...
      slide.innerHTML = `<img src="${src}" alt="Momen kebersamaan LIA & DHAFID ${i + 1}" loading="lazy" data-index="${i}">`;
  ...
  function initCalendarButtons() {
    $$('.add-to-calendar').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        const title = isAkad ? 'Akad Nikah LIA & DHAFID' : 'Resepsi LIA & DHAFID';
  ...
  function initRSVPForm() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      statusEl.textContent = `Terima kasih! Konfirmasi kehadiran Anda (${jumlah} tamu) telah kami terima.`;
